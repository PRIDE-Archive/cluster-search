package uk.ac.ebi.pride.cluster.search.model;

/**
 * @author Jose A. Dianes <jdianes@ebi.ac.uk>
 * @author ntoro
 *
 */
public class ClusterFields {
    public static final String ID = "id";
    public static final String HIGHEST_RATIO_PROTEIN_ACCESSION = "highest_ratio_protein_accession";
    public static final String HIGHEST_RATIO_PEP_SEQUENCE = "highest_ratio_pep_sequence";
    public static final String NUMBER_OF_SPECTRA = "num_spectra";
    public static final String MAX_RATIO = "max_ratio";
    public static final String CLUSTER_QUALITY = "cluster_quality";
    public static final String AV_PRECURSOR_CHARGE = "av_precursor_charge";
    public static final String AV_PRECURSOR_MZ = "av_precursor_mz";
    public static final String PROJECT_ASSAYS = "*_project";

}
