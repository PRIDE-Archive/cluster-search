package uk.ac.ebi.pride.cluster.search.model;

/**
 * @author Jose A. Dianes <jadianes@gmail.com>
 * @author ntoro
 *
 */
public class ClusterFields {

    public static final String ID = "id";
    public static final String HIGHEST_RATIO_PROTEIN_ACCESSION = "highest_ratio_protein_accessions";
    public static final String HIGHEST_RATIO_PEP_SEQUENCE = "highest_ratio_pep_sequences";
    public static final String NUMBER_OF_SPECTRA = "num_spectra";
    public static final String MAX_RATIO = "max_ratio";
    public static final String CLUSTER_QUALITY = "cluster_quality";
    public static final String AVG_PRECURSOR_CHARGE = "avg_precursor_charge";
    public static final String AVG_PRECURSOR_MZ = "avg_precursor_mz";
    public static final String PROJECT_ASSAYS = "project_assays";
    public static final String PROJECTS = "projects";
    public static final String SPECIES_NAMES = "species_names";
    public static final String SPECIES_ACCESSIONS = "species_accessions";
    public static final String SPECIES_ASCENDANTS_NAMES = "species_ascendants_names";
    public static final String SPECIES_ASCENDANTS_ACCESSIONS = "species_ascendants_accessions";
    public static final String SPECIES_NAMES_AS_TEXT = "species_as_text";
    public static final String SPECIES_ASCENDANTS_NAMES_AS_TEXT = "species_ascendants_as_text";
    public static final String CONSENSUS_SPECTRUM_MZ = "consensus_spectrum_mz";
    public static final String CONSENSUS_SPECTRUM_MZ_MEAN = "consensus_spectrum_mz_mean";
    public static final String CONSENSUS_SPECTRUM_MZ_SEM = "consensus_spectrum_mz_sem";
    public static final String CONSENSUS_SPECTRUM_INTENSITY = "consensus_spectrum_intensity";
    public static final String CONSENSUS_SPECTRUM_INTENSITY_MEAN_1 = "consensus_spectrum_intensity_mean_1";
    public static final String CONSENSUS_SPECTRUM_INTENSITY_MEAN_2 = "consensus_spectrum_intensity_mean_2";
    public static final String CONSENSUS_SPECTRUM_INTENSITY_MEAN_3 = "consensus_spectrum_intensity_mean_3";
    public static final String CONSENSUS_SPECTRUM_INTENSITY_MEAN_4 = "consensus_spectrum_intensity_mean_4";
    public static final String CONSENSUS_SPECTRUM_INTENSITY_SEM = "consensus_spectrum_intensity_sem";
}
